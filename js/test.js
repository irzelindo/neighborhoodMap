...
function ViewModel() {
  ...
  this.populateInfoWindow = function(marker, infowindow){
    ...
    if (infowindow.marker != marker) {
       ...
       var url = 'https://api.foursquare.com/v2/venues/search?ll=' +this.position.lat+ ',' +this.position.lng+
       '&client_id=' +clientID+ '&client_secret=' +clientSecret+ '&v=20130815'+ '&query=' +this.title;
       // Foursquare API
       $.getJSON(url).done(function(marker){
         self.response = marker.response.venues[0];
         self.street = self.response.location.formattedAddress[0];
         self.city = self.response.location.formattedAddress[1];
         self.phone = self.response.contact.formattedPhone;
...
function errorLoadingMap(){
  document.getElementById('map').innerHTML("Unable to load map something went wrong");
} //ViewModel ends here
